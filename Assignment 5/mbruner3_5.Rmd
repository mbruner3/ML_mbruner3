...
This linkage method looks very similar to single with the exception of the larger cluster having a few small clusters inside of it. Again I run into the same problem as with single linkage.
...
The clusters look fairly separated and fairly uniformly distributed. Cluster 6 overlaps more significantly with 3 and 5. However, looking closer at the dendrogram it does seem that it did a good job grouping the cereals into like categories. 
...
I am going with the Ward method due to the visualizations and with the strength of the structure being the highest at .905 and due to my analysis aligning with these results as well.

## Test Partitioning: Will the Clusters Have Good Stability?
```{r, message=FALSE, warning=FALSE}
# Split the dataset by 50% randomly using the Caret Package.
...
```{r, message=FALSE, warning=FALSE}
# If I used the previous one, then I would get the exact same result. I am testing to see how well the clustering algorithm will put the same cereals together in the smaller partition as the larger dataset. If the clusters are more or less the same then that will provide greater evidence that the groupings are stable.
d2 <- get_dist(test,
               method = "euclidean")
```


## Ward's Method on Test Datset
```{r, echo=FALSE}
hc_test <- hclust(d2, 
                  method = "ward.D2")
...
## Clustering Stability is Pretty Good.
```{r}
...
Looking at the above visualization starting at the leaves (on both sides), you can see it pairs the cereals together the same except for a few. It continues to maintain the groupings until the third iteration but by that time the groups at k = 6 are fairly the same. I believe this shows that the clusters are stable. The algorithm will do what I need it to do which is separate unhealthy cereals from healthy cereals.  
...
Performed the agnes hierarchical clustering and compared it with the diana hierarchical clustering to see if the clusters are similar for both methods which also gives evidence that the clusters are stable. Looking above you can see 50 of the 73 or about 70% cereals were grouped the same as agnes. I believe this gives further evidence of clustering stability as two different clustering algorithms formed mostly the same cluster groupings.  
...
# doing the same for the original dataset while also removing the incomplete cereals and "type" column to match the normalized cereal dataset.
...
# Putting cereals into their clusters and then finding the average of each cluster so I can better compare the clusters to one another and determine what kind of cereals are in each cluster.
cereal_norm %>% 
  filter(cluster == 1) %>% 
...
```{r}
# Based on the differenct recommendations for healthy amounts of fiber, sodium, and sugar. I am adding those recommendations to the original cereal dataset so I can use them in my recommendations and for comparisons of clusters.
...
All except one cereal of **cluster 2** cereals have high sodium and 11 out of 18 cereals have high sugar levels. Only 11 of the cereals have good fiber as well. 
...
* <span  style="color: #FF0000;">**Cluster 3: Not Recommended Cereals**</span><br>
  + **located on shelf 1**<br>
   <span  style="color: #FF0000;"> + **Attributes:**
    + lower sugars
    + lower fiber
    + highest sodium
    + larger serving size </span>


* <span  style="color: #17A589;">**Cluster 4: Not Recommended Cereal**</span><br>
  + **located on shelf 2**<br>
    <span  style="color: #17A589;">+ **Attributes:**
    + highest sugar
    + lowest fiber
    + lowest protein
...
13 of the cereals in **cluster 3** have healthy sugar levels. 14 cereals **do not** have healthy fiber levels and they all have high sodium. The only factor that cluster 3 has going for it is the sugar levels but I don't believe that is enough to recommend it as the other "unhealthy" factors outweigh that benefit.
...
Only 6 of the cereals in **cluster 4** have healthy sugar levels and none of them have healthy fiber or sodium levels. Again as most of the cereals have high sugar, low fiber, and high sodium levels I would not recommend the cereals in this cluster.
...
* <span  style="color: #FF0000;">**Cluster 5: Recommended Cereals**</span><br>
  + **located on shelf 3**<br>
    <span  style="color: #FF0000;">+ **Attributes:**
    + lower calories
    + lowest sodium
...
* <span  style="color: #17A589;">**Cluster 6: Not Recommended Cereals**</span><br>
  + **located on shelf 3**<br>
   <span  style="color: #17A589;"> + **Attributes:**
    + lower sugar
    + low fiber
    + higher sodium
    + higher vitamins
    + large serving size</span>

```{r}
cereal %>% 
  filter(cluster == 5) %>% 
  select(name, cluster, healthy_sugar_lvl, healthy_fiber_lvl, healthy_sodium_lvl)
```

All of **cluster 5** cereals have healthy sugar levels, 10 of the 17 cereals have healthy fiber levels, and 8 have health sodium levels. Again while not perfect, these are cereals that kids would eat and compared to the other clusters, the cereals in this cluster are overall the best. It also has a wide variety of cereals for kids to choose from and for the school to rotate.

```{r}
cereal %>% 
  filter(cluster == 6) %>% 
  select(name, cluster, healthy_sugar_lvl, high_sodium_lvl, healthy_fiber_lvl)
```

All of **cluster 6** cereals have healthy sugar levels but have high sodium and most do not have enough fiber. Although they do have high vitamins which could be a benefit but hard to determine from this dataset. Overall I don't believe that the healthy sugar levels and high vitamins are enough to recommend as the sodium levels are the highest and the fiber is lower compared to the other clusters.


## Cereal Recommendation

I recommend **cluster 5 cereals*** for the school based on the reasons listed above and I believe kids will enjoy them compared to cluster 1. Cluster 1 can be included but generally I don't believe kids would choose those cereals and for that reason I would not recommend them. <br>
