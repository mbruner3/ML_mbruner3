...
2. **Stronger negative relationships:**
...
soap2 <- soap[, c(-1,-11:-13, -27:-35, -44, -46, -50:-54)]

soap2$.data_1 <- as.factor(soap2$.data_1)
soap2$.data_2 <- as.factor(soap2$.data_2)
soap2$.data_3 <- as.factor(soap2$.data_3)
...
model2 <- glm(formula = .data_2 ~ affluence+child+cs+edu+feh+hs+sex+ave_price+pur_vol_no_promo+pr_cat_1+pr_cat_2+prop_cat_5, family = binomial, 
    data = train[, c(-35,-37)])
summary(model2)


## Predicting churn results based on the logistic model
predict_validation<-predict(model2, newdata = valid, type='response')


## Categorizing the result based on the cutoff value(0.5)
resultcheck<-as.factor(ifelse(predict_validation>0.5,1,0))
confusionMatrix(resultcheck, valid$.data_2)

model3 <- glm(formula = .data_3 ~ age+child+cs+edu+feh+hs+sec+sex+brand_runs+tot_vol+vol_tran+ave_price+pur_vol_no_promo+pur_vol_promo+pur_vol_diff_promo+pr_cat_1+pr_cat_2+pr_cat_3+pr_cat_4+prop_cat_5+prop_cat_8+prop_cat_10+prop_cat_12+prop_cat_13+prop_cat_14, family = binomial, 
    data = train[, c(-35,-36)])
summary(model3)


## Predicting churn results based on the logistic model
predict_validation<-predict(model3, newdata = valid, type='response')


## Categorizing the result based on the cutoff value(0.5)
resultcheck<-as.factor(ifelse(predict_validation>0.5,1,0))
confusionMatrix(resultcheck, valid$.data_3)
